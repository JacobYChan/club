<template>
    <div class="activity">
        <scroller lock-y
                  :scrollbar-x=false
                  ref="scroller"
                  v-if="calOfficalLength!==0">
            <div class="scroller"
                 :style="{width:calWidth+'rem'}">
                <template v-for="(value,key) in activity_official_list">
                    <router-link class="officialActivity_ceil"
                                 :to="{path:'activity/find/check/detail',query:{activityid:item.id}}"
                                 tag="div"
        ...
        <div class="private_activity">
            ...
            <div class="detail">
                <p>活动时间：2017年4月16日下午1点30分</p>
                <p>报名时间：即日起至4月15日止</p>
                <p>活动地点：兴阳商务大厦售楼处（万都装饰城对面），国展路与江阳西路交界口</p>
            </div>
        </div>
        <div class="activity_list"
             v-if="calLength!==0">
            <template v-for="(value,key) in activity_list">
                <router-link class="activity_ceil"
                             :to="{path:'activity/find/check/detail',query:{activityid:item.id}}"
                             tag="div"
                             v-for="(item,index) in value"
                             :key="index">
        ...
        <router-link tag="div"
                     to="activity/find/create/activity"
