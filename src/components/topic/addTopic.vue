<template>
    <div class="addTopic">
        ...
        <group>
            <x-input placeholder="标题(可选)"
                     v-model="value_title"></x-input>
            <x-textarea :max="200"
                        name="description"
                        placeholder="请填写有意思的内容"
        ...
        <group>
            <cell :title="title"
                  ...
                  is-link
                  @click.native="showPopup=true"></cell>
        </group>
            ...
            <div class="options">
                ...
                <template v-for="(value,key) in circles_near_list">
                    <cell :title="item.name"
                          @click.native="confirmOption(item.name,item.id)"
                          :key="index"
        ...
        <div class="sub">
            <x-button type="primary"
                      @click.native="_add_topic"
                      v-if="addClickStatus">完成</x-button>
            <x-button type="primary"
                      show-loading
                      v-else>正在上传</x-button>
        </div>
...
<script>
import { XButton, XHeader, XTextarea, Group, XInput, Actionsheet, Checker, CheckerItem, Popup, Cell, Toast, Loading, TransferDomDirective as TransferDom } from 'vux'
